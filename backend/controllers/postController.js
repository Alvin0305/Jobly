...
import {createPostFunction,getPostByIdFunction,deletePostFunction,getPostsByUserFunction,getPostsInFeedFunction} from '../models/post.js';
...
export const deletePost = async (req, res) => {
  ...
  try{
    const post_id = parseInt(req.params.id,10);
    if(isNaN(post_id)) return res.status(400).json({error:'post_id requieed'});
...
export const getPostsByUser = async (req, res) => {
  ...
  try{
    ...
    if(isNaN(user_id)) return res.status(400).json({error:'Userid invalid'})
...
export const getComments = async (req, res) => {
  ...
  try{
    const post_id = parseInt(req.params.id,10);
    if(isNaN(post_id)) return res.status(400).json({error:'Invalid post_id'});
