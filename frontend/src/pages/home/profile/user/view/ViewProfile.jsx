  ...
  useEffect ( () => {
    const fetchUserAndDetails = async () => {
      try{
        ...
        const endpoint = userData.role === "Employee"? `/workexperience/${userData.id}`: `/jobdetails/${userData.id}`;
        const exp = await axios.get(`http://localhost:5000/api/metadata${endpoint}`, {
          headers : {
            Authorization:`Bearer ${token}`,
          },
        });
      ...
      <div className="outerbox"> 
        ...
        <div className="info">
          ...
          <div className="expSection">
            <h3>{user.role === "Employee"? "Work Experience": "Job Details"}</h3>
            {workexp.length === 0 ? (
              <div>
